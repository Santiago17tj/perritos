...
const INGREDIENT_LABELS = {
  ...
  panes: "Panes",
  bebidas: "Bebidas",
  queso_tocineta: "Queso / Tocineta",
  salsas: "Salsas (frascos)",
};

const FIXED_EXPENSE_OPTIONS = ["Gas", "Arriendo del puesto", "Transporte", "Otro"];
...
export default function App() {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        const s = await window.storage.get("sales_" + today);
        if (s) setSales(JSON.parse(s.value));
        const inv = await window.storage.get("inventory");
        if (inv) setInventory(JSON.parse(inv.value));
        const exp = await window.storage.get("expenses_" + today);
        if (exp) setExpenses(JSON.parse(exp.value));
        const rst = await window.storage.get("restock_" + today);
        if (rst) setRestockLog(JSON.parse(rst.value));
        const idx = await window.storage.get("days_index");
        let arr = idx ? JSON.parse(idx.value) : [];
        if (!arr.includes(today)) {
          arr.push(today);
          await window.storage.set("days_index", JSON.stringify(arr));
        }
        arr.sort(); // ascending
        setDaysIndex(arr);
      } catch (e) {}
    ...
    async function loadReports() {
      ...
      try {
        ...
        for (const day of lastSeven) {
          const s = await window.storage.get("sales_" + day);
          const e = await window.storage.get("expenses_" + day);
          const salesArr = s ? JSON.parse(s.value) : [];
          const expArr = e ? JSON.parse(e.value) : [];
          const totalVentasDay = salesArr.reduce((a, s) => a + (s.price || 0), 0);
  ...
  async function saveSales(data) {
    setSales(data);
    await window.storage.set("sales_" + getTodayKey(), JSON.stringify(data));
  }

  async function saveInventory(data) {
    setInventory(data);
    await window.storage.set("inventory", JSON.stringify(data));
  }

  async function saveExpenses(data) {
    setExpenses(data);
    await window.storage.set("expenses_" + getTodayKey(), JSON.stringify(data));
  }

  async function saveRestockLog(data) {
    setRestockLog(data);
    await window.storage.set("restock_" + getTodayKey(), JSON.stringify(data));
  }
  ...
  function applyRestock() {
    ...
    setRestockValues({ salchichas: "", panes: "", bebidas: "", queso_tocineta: "", salsas: "" });
  ...
  const salesByProduct = PRODUCTS.map(p => ({
    ...
    total: sales.filter(s => s.id === p.id).reduce((a, s) => a + s.price, 0),
  }));
  ...
  return (
    <div style={{ fontFamily: "'Georgia', serif", background: "#1a0a00", minHeight: "100vh", maxWidth: 480, margin: "0 auto", position: "relative", paddingBottom: 80 }}>
      ...
      <div style={{ background: "linear-gradient(135deg, #c0392b 0%, #922b21 100%)", padding: "20px 20px 14px", display: "flex", alignItems: "center", gap: 12, boxShadow: "0 4px 20px #0008" }}>
        ...
        <div>
          <div style={{ color: "#f5c842", fontWeight: "bold", fontSize: 20, letterSpacing: 1 }}>Mi Perrero</div>
      ...
      <div style={{ padding: "16px 16px 0" }}>
        ...
        {tab === "ventas" && (
          <div>
            <div style={{ color: "#f5c842", fontWeight: "bold", fontSize: 16, marginBottom: 4 }}>⚡ Registrar venta</div>
                ...
                {restockLog.map((r, i) => (
                  <div key={i} style={{ padding: "8px 0", borderBottom: i < restockLog.length - 1 ? "1px solid #3d2510" : "none" }}>
                    ...
                    {Object.entries(r.items).map(([k, v]) => (
                      <div key={k} style={{ color: "#fde9a0", fontSize: 13 }}>+{v} {INGREDIENT_LABELS[k]}</div>
                    ))}
                  </div>
                ))}
              </div>
          ...
          <div style={{ background: "#1a0a00", border: "2px solid #922b21", borderRadius: "20px 20px 0 0", padding: 24, width: "100%", maxWidth: 480 }}>
            ...
            {Object.entries(INGREDIENT_LABELS).map(([key, label]) => (
              <div key={key} style={{ marginBottom: 12 }}>
                <div style={{ color: "#fde9a0", fontSize: 13, marginBottom: 4 }}>{label} (tienes: {inventory[key] || 0})</div>
                <input type="number" placeholder="Cantidad a agregar" value={restockValues[key]} onChange={e => setRestockValues(v => ({ ...v, [key]: e.target.value }))}
      ...
      {showInvEdit && (
        <div style={{ position: "fixed", inset: 0, background: "#000a", zIndex: 100, display: "flex", alignItems: "flex-end", justifyContent: "center" }}>
          <div style={{ background: "#1a0a00", border: "2px solid #922b21", borderRadius: "20px 20px 0 0", padding: 24, width: "100%", maxWidth: 480 }}>
            ...
            {Object.entries(INGREDIENT_LABELS).map(([key, label]) => (
              <div key={key} style={{ marginBottom: 12 }}>
                <div style={{ color: "#fde9a0", fontSize: 13, marginBottom: 4 }}>{label}</div>
                <input type="number" value={invEditValues[key] || 0} onChange={e => setInvEditValues(v => ({ ...v, [key]: parseInt(e.target.value) || 0 }))}
                  style={{ width: "100%", background: "#2c1a0a", border: "1px solid #5a3a1a", borderRadius: 8, padding: "10px 12px", color: "#fff", fontSize: 15, boxSizing: "border-box" }} />
              </div>
            ))}
            <div style={{ display: "flex", gap: 10, marginTop: 16 }}>
